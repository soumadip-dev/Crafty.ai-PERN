...
const RemoveBackground = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      // Extract filename
      const filename = `bgrm-${input.name}`;
  ...
  const onSubmitHandler = async e => {
    ...
    try {
      ...
      const { data } = await axios.post('/api/v1/ai/remove-image-background', formData, {
        headers: {
        ...
        {loading ? (
          ...
          <div className="min-h-[300px] sm:min-h-[400px] rounded-lg border-2 border-dashed border-gray-200 bg-gray-50 flex flex-col items-center justify-center p-4">
            <div className="flex flex-col items-center text-center">
              <Eraser className="w-8 h-8 sm:w-10 sm:h-10 text-gray-300 mb-3 sm:mb-4" />
